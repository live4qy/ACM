...
struct Tree{
    ...
    void pd(int node){
        if(laz[node]){
            a[node*2]=max(a[node*2],laz[node]);
            a[node*2+1]=max(a[node*2+1],laz[node]);
            laz[node*2]=max(laz[node*2],laz[node]);
            laz[node*2+1]=max(laz[node*2+1],laz[node]);
            laz[node]=0;
        }
    }
    void upd(int st,int ed,int val,int l,int r,int node){
        if(st<=l&&ed>=r){
            laz[node]=max(laz[node],val);
            a[node]=max(a[node],val);
            return ;
        }
        pd(node);
        int mid=(l+r)/2;
        if(st<=mid)upd(st,ed,val,l,mid,node*2);
        if(ed>mid)upd(st,ed,val,mid+1,r,node*2+1);
